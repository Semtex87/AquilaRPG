...
public class BlowGun extends CrossbowItem {
    ...
    public BlowGun(RPGTierInterface pTier, Properties pProperties) {
    ...
    public static final Predicate<ItemStack> DART_ONLY = (p_43017_) -> {
        return p_43017_.is(RPGItems.POISON_DART.get());
    };
    ...
    private static boolean tryLoadProjectiles(LivingEntity pShooter, ItemStack pCrossbowStack) {
        ...
        for(int k = 0; k < j; ++k) {
            ...
            if (itemstack.isEmpty() && flag) {
                itemstack = new ItemStack(RPGItems.POISON_DART.get());
    ...
    private static boolean loadProjectile(LivingEntity pShooter, ItemStack pCrossbowStack, ItemStack pAmmoStack, boolean pHasAmmo, boolean pIsCreative) {
        if (pAmmoStack.isEmpty()) {
            ...
            boolean flag = pIsCreative && pAmmoStack.getItem() instanceof PoisonDart;
            ItemStack itemstack;
    ...
    private static void shootProjectile(Level pLevel, LivingEntity pShooter, InteractionHand pHand, ItemStack pCrossbowStack, ItemStack pAmmoStack, float pSoundPitch, boolean pIsCreativeMode, float pVelocity, float pInaccuracy, float pProjectileAngle) {
        if (!pLevel.isClientSide) {
            ...
            pLevel.playSound((Player)null, pShooter.getX(), pShooter.getY(), pShooter.getZ(), RPGSoundEvents.BLOWGUN_USE.get(), SoundSource.PLAYERS, 1.0F, pSoundPitch);
        }
    }
    private static AbstractArrow getArrow(Level pLevel, LivingEntity pLivingEntity, ItemStack pCrossbowStack, ItemStack pAmmoStack) {
        PoisonDart PoisonDart = (PoisonDart)(pAmmoStack.getItem() instanceof PoisonDart ? pAmmoStack.getItem() : RPGItems.POISON_DART);
        AbstractArrow abstractarrow = PoisonDart.createArrow(pLevel, pAmmoStack, pLivingEntity);
