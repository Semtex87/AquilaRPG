...
public class MusketGun extends CrossbowItem {
    ...
    public MusketGun(RPGTierInterface pTier, Properties pProperties) {
    ...
    public static final Predicate<ItemStack> BULLET_ONLY = (p_43017_) -> p_43017_.is(RPGItems.BULLET.get());
    ...
    private static void shootProjectile(Level pLevel, LivingEntity pShooter, InteractionHand pHand, ItemStack pCrossbowStack, ItemStack pAmmoStack, float pSoundPitch, boolean pIsCreativeMode, float pVelocity, float pInaccuracy, float pProjectileAngle) {
        if (!pLevel.isClientSide) {
            ...
            pLevel.playSound((Player)null, pShooter.getX(), pShooter.getY(), pShooter.getZ(), RPGSoundEvents.MUSKET_USE.get(), SoundSource.PLAYERS, 1.0F, pSoundPitch);
        }
    }
