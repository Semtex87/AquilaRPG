...
public class DaggerEntity extends AbstractArrow implements IEntityAdditionalSpawnData {
    ...
    protected void onHitEntity(EntityHitResult hitResult) {
        ...
        float damage = ((RPGDaggerWeapon) daggerItem.getItem()).getActualAttackDamage();
        float impalingDamage = (int) ((RPGDaggerWeapon) daggerItem.getItem()).getArmorPiercing() * ticksSpinning;
            ...
            if (living != null) {
                if (!(living.getMobType().equals(MobType.UNDEAD))) {
                    if (!(living.isBlocking())) {
                        if (living.getArmorValue() <= 9.0F && new Random().nextInt(3) == 1) {
                            living.playSound(RPGSoundEvents.SWORD_IMPACT.get(), 1.5F, 1.0F);
                            living.hurt(src, impalingDamage);
                            Log.info(living + " is impaled with " + impalingDamage + " damage" );
